...
# 題號, 正確答案, 機器人答案, 第一次作答時間, 第一次受測者答案, 第二次作答時間, 第二次受測者答案
PRETEST = ['B', 'C', 'C', 'B', 'B', 'D']
...
def pretest_data(filename, difficulty, answers):
    ...
    with open(f"{filename[:-4]}.csv", 'w', newline='') as output:
        writer = csv.writer(output)
        writer.writerow(["題號", "難度", "正確答案", "作答時間(ms)", "受測者答案"])
        for d in data:
            writer.writerow(d)
...
def test_data(filename, difficulty, answers, robots):
    ...
    with open(f"{filename[:-4]}.csv", 'w', newline='') as output:
        writer = csv.writer(output)
        writer.writerow(["題號", "難度", "正確答案", "機器人答案", "第一次作答時間(ms)", "第一次受測者答案", "第二次作答時間(ms)", "第二次受測者答案"])
        for d in data:
            writer.writerow(d)

def output_csv(filename):
    test_type = filename[10:-4]
...
filename = input("filename(e.g. 0528-1553-pretest.txt): ")
output_csv(filename)